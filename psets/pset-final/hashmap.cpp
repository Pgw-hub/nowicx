...
bool isprime(int x) {//많이 사용하지 않지만 일단 완성되어있음.
...
string hashspecs(unordered_map<string, int> ht) {//완성
	stringstream ss;
...
bool ends_with(const std::string& str, const std::string& sub) {//완성
	return str.size() >= sub.size() &&
		str.compare(str.size() - sub.size(), sub.size(), sub) == 0;
}

void show_timeit(int begin) { 	// display elapsed time//완성
...
bool insert(unordered_map<string, int>& ht, string key) {//완료
	DPRINT(cout << ">insert: " << key << endl;);
    // use find() in unordered_map to check whether or not key exists.
    unordered_map<string,int>::iterator find_it;
    find_it = ht.find(key);
...
bool erase(unordered_map<string, int>& ht, string key) {//완료
	DPRINT(cout << ">erase: " << key << endl;);

    // calls unordered_map erase() and returns its return value: true or false.
    //unordered_map<string,int>::iterator result;
    int result = ht.erase(key);
...
void doit(bool (*func_ptr)(unordered_map<string, int>&, string), unordered_map<string, int>& ht) {//완료
	string key, word;
	clock_t begin = 0;
...
void show_range(unordered_map<string, int>& ht, vector<int>& bucket_idx, size_t from, size_t to) {
    for(auto i = from; i < to; i++){
        int x = bucket_idx[i];
        cout << "\t[" << x << "] ";
        for(auto it = ht.begin(x); it!= ht.end(x); it++){
            cout << it->first << " : " << it->second << "\t";
        }
        cout << endl;
    }
}//이렇게 할 경우에 전부 다 출력함. 따라서 from , to 를 사용해야함
void show(unordered_map<string, int>& ht, bool show_empty = false, size_t show_n = 10) {//완료
