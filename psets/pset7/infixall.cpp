...
double evaluate(string tokens) {//기준
	...
	for (size_t i = 0; i < tokens.length(); i++) {//한줄로 쭉 받고, 아까처럼 char로 받는게 아니라, 
		...
		// current token is a value(or operand), push it to va_stack.
		else if (isdigit(tokens[i])) {//그 자리가 숫자일경우에,
            ...
            double answer =0;
			cout << "hello [ ')' ]" << endl;
			while(op_stack.top() != '('){//top이 왼쪽괄호이때까지.
				cout << "i'm doinig this!"<<endl;
				answer = compute(va_stack,op_stack);
			}
			cout << "op_stack_top =" << op_stack.top() << endl;
			cout << "parenthesis value = " << answer << endl;
			op_stack.pop();
		}
			...
			else if(precedence(tokens[i]) > precedence(op_stack.top())){//우선순위가 높을 경우는 oprand
				op_stack.push(tokens[i]);
				//cout << op_stack.top() << endl;
				//cout << "compute 전 op_stack_size : " << op_stack.size() << endl;
				//compute(va_stack,op_stack);
				//cout << "compute 후 op_stack_size : " << op_stack.size() << endl;
				//cout << va_stack.top() << endl;
				
				//cout << "op_stack_top : " << op_stack.top() << endl;
			}
			else{//precedence가 일반적일떄.
				compute(va_stack, op_stack);
				op_stack.push(tokens[i]);
			}
		...
		cout << "op_size()" << op_stack.size() <<endl;
