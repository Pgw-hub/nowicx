...
graph graph_by_file(char* fname) {//완료
	string filename = fname;
	return graph_by_file(filename);
}

graph graph_by_file(string fname) {//완료
	int V, E;
	int v1, v2;
	int error;
	fstream fp;
...
bool empty(graph g) {//완료
	if (g == nullptr || g->V == 0) return true;
	return false;
}

bool validVertex(graph g, int s) {//완료
	if (empty(g)) return false;
	if (s >= 0 && s < g->V) return true;
	return false;
}

int V(graph g) { return g->V; }//완료		// returns number of vertices
int E(graph g) { return g->E; }//완료				// returns number of edges
string gfile(graph g) { return g->gfile; }//완료	// returns graph filename

int degree(graph g) {//완료
	int max = 0; 
	for (int v = 0; v < V(g); ++v) {
		int deg = degree(g, v);
		if (deg > max) max = deg;
...
void addEdgeFromTo(graph g, int v, int w) {//완료
...
void addEdge(graph g, int v, int w) { // 완료
...
void setBFS0(graph g, int v, queue<int>& que) {//완료
...
void BFS(graph g, int v) {//완료
	...
	for (int i = 0; i < V(g); i++)  g->marked[i] = false;//마크
	g->parentBFS[v] = -1;//시작은 부모 -1
	g->marked[v] = true;//시작햇으니
	g->distTo[v] = 0;//처음-처음
	g->BFSv = {};    //bfsv
...
void setDFS0(graph g, int v, queue<int>& que) {//완료
...
void DFS(graph g, int v, queue<int>& que) {  //완료
...
void DFS_CCs(graph g) {//완료
    ...
    for(int i=0; i<V(g); i++){
        ...
        g->CCID[i] = ccid;
			  ...
			  // set results into DFS0[] and CCID[]
...
void DFSpath(graph g, int v, int w, stack<int>& path) {//완료
	...
	queue<int> q;//path를 넣을 큐를 선언.
	DFS(g, v, q);//다시 dfs를 하는데, 시작점을 v부터.  	             // DFS at v, starting vertex
	g->DFSv = q;
	path = {};                   // clear path, stack<int>().swap(path);
    
    //도착지를 먼저 i로 설정해줌.
    //For문 내에서 i 를 도착지의 부모로 설정을 해줌.
    for(int i = w; i!= v; i = g->parentDFS[i]){
        path.push(i);
    }
    path.push(v);
...
// It has to use a stack to retrace the path back to the source.
// Once the client(caller) gets a stack returned, 
void BFSpath(graph g, int v, int w, stack<int>& path) {//완료
	...
	path = {};                   // clear path
    for(int i = w; i!= v; i = g->parentBFS[i]){
        path.push(i);
    }
    path.push(v);
...
bool connected(graph g, int v, int w) {//완료
...
int distTo(graph g, int v, int w) {//완료
	if (empty(g)) return 0;
	if (!connected(g, v, w)) return 0;

	BFS(g, v);
    int destination = g->distTo[w];
    int departure = g->distTo[v];
    

    int dist = destination - departure;
	return dist;
}
